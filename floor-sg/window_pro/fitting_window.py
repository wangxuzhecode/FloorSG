...
doorpath = r'E:\Stanford3dDataset_v1.2\area4_window'
# fig = plt.figure(figsize=(10, 7))
windowpath = r'E:\Stanford3dDataset_v1.2\area1_window'
fig, ax = plt.subplots()

files = os.listdir(doorpath)
window_files = os.listdir(windowpath)
print(files)
...
#     print(i, filename)
#     matrix = []
#     with open(os.path.join(windowpath, filename), 'r') as file:
#         for line in file:
#             # 将每行的数字按空格或制表符分割，并转换为float类型，存储为一行矩阵
#             row = list(map(float, line.split()))
#             matrix.append(row)
...
#         start_point = np.array([x_fit[0], y_min])
#         end_point = np.array([x_fit[-1], y_max])
#         window_line.append((start_point, end_point))
#         plt.plot([start_point[0], end_point[0]], [start_point[1], end_point[1]], 'r--', label='Extended Line')
#     else:
#         x_fit = np.linspace(x_min, x_max, 100)
#         y_fit = m * x_fit + b
#
...
# output_path = '109_door.txt'
# height = 2
# hash_wall = [0 for _ in wall_line]
# wall_label = {}
# print(len(wall_line))
# with open(output_path, 'w', encoding='utf-8') as f:
#     for line in door_line:
#         projected_line, selected_idx = project_line_to_closest_segment(wall_line, line)
#
#         print(selected_idx)
...
#         f.writelines(cur_str)
#         f.write('\n')
#     for line in window_line:
#         projected_line_window, selected_idx_window = project_line_to_closest_segment(wall_line, line)
#         plt.plot([projected_line_window[0][0], projected_line_window[1][0]], [projected_line_window[0][1], projected_line_window[1][1]], 'g', label='Extended Line')
#
#
# print(len(wall_line))