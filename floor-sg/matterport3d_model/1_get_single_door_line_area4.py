...
def fit_line(points, thres, original_point,out=0.2):
    ...
    if max(y_fit[-1], y_fit[0])>y_max+0.4 or min(y_fit[-1], y_fit[0])<y_min-0.4:
        print(1111111111)
        new_x_min = (y_min - b) / m
        new_x_max = (y_max - b) / m
    ...
    return st_pos, ed_pos, outliers, m,b
...
def fit_one_door(points):
    ...
    st_pos1, ed_pos1, outliers1, m1,b1 = fit_line(points, thres=0.1,original_point=points, out=0.25)
    # ax.scatter(points[:,0], points[:,1], color='k',s=1)
    # #
    # ax.scatter(outliers1[:,0], outliers1[:,1], color='r',s=1)

    line_lst.append((st_pos1, ed_pos1))
    param_lst.append((m1, b1))
    # print(y_length,x_length)
    if min(y_length, x_length)>0.6:
        if outliers1.shape[0]<10:
            return line_lst, param_lst
        print(outliers1.shape)
        st_pos2, ed_pos2, outliers2, m2,b2 = fit_line(outliers1, thres=0.032,original_point=points)

        # ax.scatter(outliers2[:, 0], outliers2[:, 1], color='y', s=1)
        param_lst.append((m2, b2))
        line_lst.append((st_pos2, ed_pos2))
        if outliers2.shape[0]>15:
            st_pos3, ed_pos3, outliers3,m3,b3 = fit_line(outliers2, thres=0.03,original_point=points)
            if are_segments_similar(st_pos1[0], st_pos1[1], ed_pos1[0], ed_pos1[1], st_pos3[0], st_pos3[1], ed_pos3[0], ed_pos3[1], 2, 1)==False:
                if are_segments_similar(st_pos2[0], st_pos2[1], ed_pos2[0], ed_pos2[1], st_pos3[0], st_pos3[1], ed_pos3[0], ed_pos3[1], 2, 1)==False:
                    ...
                    param_lst.append((m3, b3))

    return line_lst, param_lst

doorpath = r'E:\matterport3d_used\scene3_single_door_sub'
...
for i, filename in enumerate(files):
    ...
    z_min = np.min(pointcloud_per_room,0)[2]
    idx = np.argwhere((pointcloud_per_room[:,2]>1.8+z_min))
    points = pointcloud_per_room[idx]
    points = points[:, 0, 0:2]
    if points.shape[0]==0:
        continue
    line_lst, param_lst = fit_one_door(points)
    pointcloud_per_room = pointcloud_per_room[:, 0:2]
    min_v, min_idx = 1e8, -1
    max_length, min_length = 0, 1e7
    max_len_idx = -1
    for i, line in enumerate(line_lst):
        ...
        cur_b = param_lst[i][1]
        t = 0
        cur_length = math.sqrt((line[0][0]-line[1][0])**2 + (line[0][1]-line[1][1])**2)
        inliers = np.array([point for point in points if distance(point, cur_m, cur_b) < 0.01])
        cover = cover_len(inliers)
        print(cover)
        if cover_len(inliers)<0.5:
            continue
        for point in pointcloud_per_room:
            if distance(point, cur_m, cur_b)<0.01:
                t+=1
        print(t)
        if t < min_v:
            min_v = t
    ...
    if max_length-min_length>1:
        line = line_lst[max_len_idx]
    else:
        line = line_lst[min_idx]
