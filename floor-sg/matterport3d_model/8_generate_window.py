...
def save_mtl(filepath, file_name):
    ...
    with open(filepath, "w") as mtl_file:
        ...
        mtl_file.write("map_Kd window.jpg\n")

path_roomBound = 'scene3_project_window.txt'
path_save_mesh = 'E:\\FloorSG\\code\\S3DIS_total\\data\\scene3\\window\\'
window_mesh = []
...
for i, room in enumerate(new_data):
    print(room.shape)
    A = np.array([room[0], room[1], 0.7])  # 线段A的坐标 (x1, y1, z1)
    B = np.array([room[3], room[4], 0.7])  # 线段B的坐标 (x2, y2, z2)
    height = 2  # 给定的高度
    top_A = A + np.array([0, 0, 1.3])  # 顶部A
    top_B = B + np.array([0, 0, 1.3])  # 顶部B
    ...
    # window_mesh.append(mesh)
    # mesh.save(path_save_mesh +str(i)+ ".obj")
# combined_mesh = window_mesh[0]
# for mesh in window_mesh[1:]:
