...
from open3d.cpu.pybind.core import append
...
def get_points_by_roomline(roomline, points, theta = 20):
    gridsize = 0.08
    min_data = [-37.928000000000000, -23.435500000000000]
    height = 344
    width = 570
...
def get_ransac_line(selected_points, room_range, mode, points = None):
    ...
    if mode == 1:
        ...
        x_min = np.min(points, 0)[0] - 0.2
        x_max = np.max(points, 0)[0] + 0.2
        x_fit = np.linspace(x_min, x_max, 100)
        y_fit = m * x_fit + b
...
def remove_similar_lines(lines, thres_dist, thres_angle, points):
    ...
    for i,line1 in enumerate(lines):
        for j, line2 in enumerate(lines):
            ...
            if calculate_dist_lines(line1, line2) ==0 and calculate_abs_slope(line1, line2) < thres_angle:
...
if __name__ == '__main__':
    ...
    area_name = r'E:\FloorSG\Expriment_output\S3DIS\Boundary_optim\area5.txt'
    ...
    point_path = r'E:\Stanford3dDataset_v1.2\Area_5\vertical_sub.txt'
    output_path = r'C:\2024\FloorSG\S3DIS_vector\area5'
    matrix = []
    ...
    for i in range(51, 52):
        ...
        num_edge = 0
        cur_pos = boundary_lst[10]
        hash_map = np.zeros_like(cur_boundary_map)
        segments = []
        while 1:
            if num_edge == len(boundary_lst) or (num_edge>=len(boundary_lst)-10 and cur_pos==boundary_lst[10]):
                break
            print(cur_pos)
        ...
        for num, seg in enumerate(merged_segments):
            ...
            if length<6:
                buffer_points.append(selected_points)
                continue
            if length >= 6 or num == len(merged_segments)-1:
                if num == len(merged_segments)-1:
                    print(len(buffer_points))
                if len(buffer_points)!=0:
                    selected_points1 = np.vstack(buffer_points)
                    new_start_point, new_end_point, k = get_ransac_line(selected_points1, None, 3, selected_points1)
                    if k == None:
                        continue
                    cur_points.append(selected_points1)
                    cur_room_lineseg.append((new_start_point[0], new_start_point[1], new_end_point[0], new_end_point[1]))
                    pointcloud = np.concatenate([pointcloud, selected_points1], axis=0)
                    buffer_points = []
            print(selected_points.shape)
        ...
        if len(buffer_points)!=0:
            ...
            print('2222', new_start_point, new_end_point)
            cur_points.append(selected_points1)
            cur_room_lineseg.append((new_start_point[0], new_start_point[1], new_end_point[0], new_end_point[1]))
            pointcloud = np.concatenate([pointcloud, selected_points1], axis=0)
        ...
        cur_room_lineseg = remove_similar_lines(cur_room_lineseg, thres_dist=0.1, thres_angle=8, points=np.vstack(cur_points))
        ...
        for segment in res_line:
            ...
            plt.plot([segment[0][0], segment[1][0]], [segment[0][1], segment[1][1]], color=cur_line_color,
                     label='Split Segment')
        for point in inter_points:
            plt.scatter(point[0], point[1], color='black')  # 交点

        for p in cur_points:
            plt.scatter(p[:, 0], p[:, 1], color=random_color(), label='Points', s=2)
