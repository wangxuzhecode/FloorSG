...
def get_points_by_roomline(roomline, points, theta = 120):
    gridsize = 0.22/4
    min_data = [-10.876497268676758, -10.849895477294922]
...
def ransac_line_fitting(points, iterations=3000, threshold=0.02):
    best_inliers = []
    best_m = None
    best_b = None
    ...
    for _ in range(iterations):
        ...
        m = (y2 - y1) / (x2 - x1)
        b = y1 - m * x1
        if abs(min(m, 1/(m+1e-9))) > 1 / 2000:  # 约束斜率
            continue
        if m==0:
            continue
...
def calculate_length_line(line):
    p1 = (line[0], line[1])
    p2 = (line[2], line[3])
...
def remove_similar_lines(lines, thres_dist, thres_angle, points):
    ...
    for i,line1 in enumerate(lines):
        # if calculate_length_line(line1)>30:
        #     continue
...
if __name__ == '__main__':
    ...
    area_name = r'E:\FloorSG\Expriment_output\Matterport3d\JeFG25nYj2p_filter_resize.txt'
    ...
    point_path = r'E:\matterport3d_used\JeFG25nYj2p\vertical_sub.txt'
    output_path = r'C:\2024\FloorSG\S3DIS_vector\scene1'
    matrix = []
    ...
    for i in range(11, 12):
        ...
        while 1:
            if num_edge == len(boundary_lst):
                break
            # if num_edge >= len(boundary_lst)-5 :
            #     break
            ff = False
        ...
        for num, seg in enumerate(merged_segments):
            ...
            # with open(roomline_path, "a") as f
            #     res_st, res_ed = change_points((seg[0], seg[1]), (seg2[2],seg[3]))
            #     f.write(f"{res_st[1]}\t{res_st[0]}\t{res_ed[1]}\t{res_ed[0]}\t{11}\t{11}\n")

        cur_path = os.path.join(output_path, str(i))
        if not os.path.exists(cur_path):
            os.makedirs(cur_path)
        cur_room_lineseg = remove_similar_lines(cur_room_lineseg, thres_dist=0.05, thres_angle= 1, points=np.vstack(cur_points))
